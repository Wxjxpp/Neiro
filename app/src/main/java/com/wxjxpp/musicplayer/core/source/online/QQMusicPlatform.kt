 ...
 * 网易云音乐平台适配。
 *
 * 搜索：`music.163.com/api/search/get/web`（需要 Referer，否则 403）
 * 歌词：`music.163.com/api/song/lyric`，`lv/tv/rv` 分别对应原文 / 翻译 / 罗马音，
 *       其中 `klyric` 是网易自家的逐字格式（`[时间,时长]` + 逐字），
 *       正好能被本项目的 LrcParser 直接吃下。
 */
internal class NeteasePlatform(http: HttpClient) : BasePlatform(http) {
    ...
    private val headers = mapOf(
        ...
        "Cookie" to "appver=8.9.70; os=android; NMTID=1",
    )

    override suspend fun search(keyword: String, page: Int, pageSize: Int): List<Song> {
        val offset = (page - 1).coerceAtLeast(0) * pageSize
        val url = "https://music.163.com/api/search/get/web?s=${keyword.urlEncoded()}" +
            "&type=1&offset=$offset&limit=$pageSize"
        val response = http.get(url, headers)
        if (!response.isSuccessful) return emptyList()
        val root = runCatching { JSONObject(response.body) }.getOrNull() ?: return emptyList()
        val list = root.optJSONObject("result")?.optJSONArray("songs") ?: return emptyList()

        return list.objects().mapNotNull { item ->
            val songId = item.optLong("id").takeIf { it > 0 }?.toString() ?: return@mapNotNull null
            val album = item.optJSONObject("album")
            // 封面要靠专辑 picId 拼，搜索结果里没有直接的图片地址
            val cover = album?.optLong("picId")?.takeIf { it > 0 }
                ?.let { "https://p1.music.126.net/img/album/$it.jpg" }
                ?: album?.optJSONObject("artist")?.optString("img1v1Url")?.takeIf { it.isNotBlank() }
            buildSong(
                songId = songId,
                title = item.optString("name"),
                artist = item.optJSONArray("artists").joinNames(),
    ...
    override suspend fun lyrics(songId: String, payload: String?): PlatformLyrics? {
        val url = "https://music.163.com/api/song/lyric?id=$songId&lv=1&tv=1&rv=1&kv=1"
        val response = http.get(url, headers)
        if (!response.isSuccessful) return null
        val root = runCatching { JSONObject(response.body) }.getOrNull() ?: return null
        val lyric = root.optJSONObject("lrc")?.optString("lyric")?.takeIf { it.isNotBlank() }
            ?: return null
        return PlatformLyrics(
            lyric = lyric,
            ...
            // klyric 是逐字格式，交给 LrcParser 识别
            lxlyric = root.optJSONObject("klyric")?.optString("lyric")?.takeIf { it.isNotBlank() },
        )
    }
}